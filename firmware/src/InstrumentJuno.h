...
struct JunoState {
    ...
    void loadFromSysex(int patchIndex) {
        ...
        // Continuous sliders (0-127 mapped to 0.0f .. 1.0f)
...
private:
    ...
    float _param_pwm_pct = 50.0f;
    float _param_saw_pct = 100.0f;
    float _param_sub_pct = 50.0f;
    float _param_noise_pct = 0.0f;
    float _param_cutoff_pct = 50.0f;
    float _param_res_pct = 20.0f;
    float _param_lfo_pct = 0.0f;

    JunoState state;
    ParamDescriptor _junoParams[MAX_PARAMS];
    uint8_t _junoParamCount = 0;

    void syncStateToParams();
